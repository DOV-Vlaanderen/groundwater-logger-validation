...
zdr <- gwloggeR:::model_drifts.simulate(n = length(ts.dr), mu = 1033, phi1 = 0.85, sigma = sqrt(25), xreg = as.matrix(dt), betas = c(3))
...
For outlier detection in air pressure data we used a simple gaussian noise model: it is simple and easy to interpret. For drift detection this gaussian noise model is inadequate. The problem is the correlation between subsequent observations, which directly affects significance calculation of the drift.
...
```{r, echo=FALSE, fig.cap='Having $\\phi$ smaller than 1 (left) results in a stationary series around $\\mu$, while a random walk (right) will have no bounds. Both series have variance equal to 25.'}
set.seed(2021)
