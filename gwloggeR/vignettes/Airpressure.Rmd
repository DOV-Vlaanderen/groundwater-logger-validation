...
vignette: >
  %\VignetteIndexEntry{Air pressure}
...
This is an advanced guide. It explains how `detect_outliers()` works for air pressure data. Make sure you have first read the [getting started](gwloggeR.html) material.
...
saveRDS(df, 'airpressure/simulation.rds')
```
...
```{r detect_outliers_norm_power, echo=FALSE}
df <- readRDS('airpressure/simulation.rds')
...
In case of outliers, a way to measure robustness is the breakdown point. The maximum attainable breakdown point is $50 \%$, meaning that $50 \%$ of observations can be replaced by arbitrary large numbers, without breaking the estimator. For $\mu$, the obvious choice is the median. For $\sigma$ it is the median absolute deviation (MAD). They both have a $50 \%$ breakdown point. A problem with the latter is its efficiency (cf. simulation). There exist more efficient $\sigma$-estimators than the MAD. For example the $Qn$-estimator [cf. @Rousseeuw1993]. Currently, MAD suffices because we usually have lots of data ($n > 5000$) so efficiency suffers less.

### Apriori air pressure information

We can improve the outlier detection by providing _a-priori_ information about $\mathbf{x}$. For example:

```{r detect_outliers-apriori-plot}
x <- c(990:999)
gwloggeR::detect_outliers(x, apriori = gwloggeR::apriori("air pressure", "cmH2O"), plot = TRUE)
```

Top left is again the histogram of $\mathbf{x}$. But the green density this time is not a robust normal estimate based on $\mathbf{x}$. Instead it is the hard-coded _a-priori_ density distribution of air pressure ($cmH_2O$) in Belgium. Given the $10$ points and assuming that we want to detect falsely one or more outliers in $1$ of $2000$ tests, we set the red cutoffs appropriately. This results in first $4$ points being identified as outliers.

## Future work

* `detect_outliers(x)` based on $Q_n$ estimator for variance.
* Option to supply elevation of the air pressure logger.
