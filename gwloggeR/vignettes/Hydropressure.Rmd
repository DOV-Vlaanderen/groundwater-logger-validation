...
The main idea is that $z_t$ is polluted by different kinds of events. We define each of these events seperatey. Inspiration of these definitions is mainly taken from the works of @Fox1972 and more recent papers by @Chen1993 and @Galeano2013.
...
The $I()$ is the indicator function. Furthermore, $x_t$ is the observed value, and $z_t$ is the underlying process as defined previously. $z_t$ is "polluted" in this case with $\omega$ at time $t = t_{AO}$. Rewriting this in function of $x$ we get:

$$
x_t = x_{t-1} + \epsilon(\Delta t) + \omega \left[I(t=t_{AO}) - I(t = t_{AO} + 1) \right]
$$

These equations in function of $x$ we need later for likelihood maximization.
...
This model selection is quite involved. Programaticaly, we define an object `ProgressTable` which keeps track of all the best models $M_0$ up till the last significant one. Besides the forward model selection, we also use a backward selection because adding a parameter sometimes makes an exising parameter insignificant, which opens a new path of possibilities in case it is removed. The eventual model $M_m$ is the most likely model with $m$ parameters of all the ones tested, but not necessarily the most likely of all the possible configurations. In practive, although this maxima might be local (i.e. not global), it is taken as a good compromise between compuational speed and accuracy.

### Likelihood optimization

Each time we add a parameter, our likelihood function changes and must be optimized for most likely $\mathbf{\theta}$. Using a smoothing kernel for $\epsilon(\Delta t)$ results in non-convex likelihood. Assuming $\epsilon(\Delta t)$ to be normal has several advantages: speed, numerical stability (due to $logL(\mathbf{\theta} ; \mathbf{x})$) and "mostly" convexity. Experience shows that mixing AO, LS and TC events sometimes results in non-convex likelihoods.
