local({
  ...
  fn <- function(f) {
    ...
    saveRDS(tso, paste0('./tsoutliers/tsoutliers_inbo/', basename(f), '.tsoutliers'), ascii = TRUE, compress = FALSE)

    local({
      png(paste0('./tsoutliers/tsoutliers_inbo/', basename(f), '.png'), width = 1280, height = 720)
      on.exit(dev.off())
